...
def fetch_live_scores_for_picks(games):
    ...
    for league_name, league_games in games_by_league.items():
        ...
        # Bucket by the game's Eastern calendar date, not its raw UTC date --
        # ESPN's own "dates" param buckets a night game (e.g. an 8:15pm ET
        # kickoff, already past midnight UTC) under the Eastern date, so a
        # bare .strftime() on a UTC-aware datetime lands one day late and
        # silently never finds primetime games at all.
        ...
        if not games_in_week:
            ...
            with st.spinner("Pulling final scores from ESPN..."):
                ...
                for league_name, league_games in games_by_league.items():
                    ...
                    # week's span.
                    # IMPORTANT: bucket by the game's Eastern calendar date, not its
                    # raw UTC date -- ESPN's own "dates" param buckets a night game
                    # (e.g. an 8:15pm ET kickoff, already past midnight UTC) under
                    # the Eastern date, so a bare .strftime() on a UTC-aware
                    # datetime lands one day late and silently never finds
                    # primetime games at all.
                    unique_dates = sorted({d.astimezone(ZoneInfo("America/New_York")).strftime("%Y%m%d") for d in kickoff_dates})
...
with tab_players:
    ...
    if not players_for_payment:
        ...
        edit_payment_status = st.checkbox("✏️ Edit payment status", key="edit_payment_status")

        if not edit_payment_status:
            def _payment_method_label(p):
                if not p.get("paid"):
                    return "—"
                method = p.get("payment_method") or "—"
                if method == "Other" and p.get("payment_method_other"):
                    return f"Other ({p['payment_method_other']})"
                return method
                ...
                [
                    {
                        ...
                        "Payment Method": _payment_method_label(p),
                    }
